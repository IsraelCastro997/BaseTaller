          ...
          function Envio(){   
			document.getElementById("date").value=variableJs;
              if (confirm('Enviar datos?')) {
                  ...
                  document.Forma01.action = 'herramientas_salva.php';
...
<script>
	...
	function agregar(){
		...
		<?php
             $sentencia = $pdo->prepare("SELECT * FROM herramientas  WHERE eliminado = 0");
             $sentencia->execute();
             $lista=$sentencia->fetchAll(PDO::FETCH_ASSOC);
             	...
             	foreach ($lista as $mostrar) { 
                    ...
                    ?>
						...
						'</td><td><?php echo $mostrar['id']?></td><td><?php echo $mostrar['nombre']?></td><td><?php echo $mostrar['estatus']?></td><td><?php 
						echo $mostrar['fechaEntrada']?></td><td><?php echo $mostrar['medida'] ;?></td><td><?php echo $mostrar['ubicacion'] ;?></td><td><?php
						echo "<form name=\"borrar\"  method=\"post\" action=\"herramientas_elimina.php?id=".$mostrar['id']."\"><input onclick=Borrar(); return=\" false;\" type=\"submit\" class=\"btn btn-danger\" value=\"Eliminar\"></form>";?></td><td><?php 
						echo "<a href=\"herramientas_edita.php?id=".$mostrar['id']."\" class=\"btn btn-success\"> Editar</a>";?></td><td><?php 
						echo "<a href=\"herramientas_detalles.php?id=".$mostrar['id']."\" class=\"btn btn-secondary\">  detalles</a>";?></td></tr>';
						$('#tabla').append(fila);
						reordenar();
                <?php
                $numero++;
...
<body>
<form  name="Forma01" method="POST" enctype="multipart/form-data" action="herramientas_salva.php">
		<?php
			include("menu.php");
		?>
	...
	<div id="content">Lista De Herramientas
		<br>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          <label>
          Estado:
          <input id="estatus" type="text" name="estatus" placeholder="Escribe el estado" required class="form-control">
          </label>
        </div>
        <div class="form-grou">
